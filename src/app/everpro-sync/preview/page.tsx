      ...
      try {
        ...
        const matrix = jsonData.map(row => {
          ...
          return Array.from({ length: maxCols }, (_, i) => ({
            value: rowArr[i] === undefined || rowArr[i] === null ? "" : String(rowArr[i]),
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-white overflow-hidden font-sans">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-3 shadow-sm z-10">
        <div className="flex items-center gap-4">
          <Link href="/everpro-sync">
            <Button variant="outline" size="sm" className="h-9 w-9 p-0 rounded-xl transition-all hover:bg-slate-50">
          ...
          <div>
            <h1 className="text-sm font-bold text-slate-900 truncate max-w-[300px]">{fileName}</h1>
            <div className="flex items-center gap-2">
               <span className="flex h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
               <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Live Preview • Optimized for Web</span>
            </div>
          </div>
        </div>
           ...
           {!loading && !error && (
              <div className="hidden sm:flex items-center gap-6 mr-4 border-r border-slate-100 pr-6">
                 <div className="text-center">
                    <p className="text-[10px] font-bold text-slate-400 uppercase leading-none">Rows</p>
                    <p className="text-sm font-black text-slate-900 leading-tight">{data.length}</p>
                 </div>
                 <div className="text-center">
                    <p className="text-[10px] font-bold text-slate-400 uppercase leading-none">Status</p>
                    <p className="text-sm font-black text-emerald-600 leading-tight">Synced</p>
                 </div>
              </div>
           )}
           <Badge tone="info" className="px-3 py-1 font-bold text-[10px]">READ ONLY MODE</Badge>
        </div>
      </div>
      ...
      <div className="relative flex-1 bg-[#f8fafc] overflow-hidden">
        {error ? (
          <div className="flex h-full flex-col items-center justify-center p-6 text-center">
            ...
            <h2 className="text-xl font-black text-slate-900">Preview Unavailable</h2>
            <p className="mt-2 text-slate-500 max-w-xs">{error}</p>
            <Link href="/everpro-sync" className="mt-8">
               <Button className="rounded-2xl px-8 shadow-lg shadow-blue-500/20">Return to History</Button>
            </Link>
          </div>
        ) : (
          <div className="h-full w-full overflow-auto custom-scroll p-4 sm:p-8">
            {loading ? (
              <div className="flex h-full w-full flex-col items-center justify-center">
                 ...
                 <p className="mt-6 text-sm font-black text-slate-400 tracking-widest uppercase animate-pulse">Building Grid Matrix...</p>
              </div>
            ) : (
      ...
      <style jsx global>{`
        ...
        .Spreadsheet-row-header, .Spreadsheet-column-header {
...
export default function PreviewPage() {
  return (
    <Suspense fallback={<div className="h-screen w-screen flex items-center justify-center text-slate-400 font-bold uppercase tracking-widest bg-white">Booting Preview...</div>}>
      <PreviewContent />
    </Suspense>
  );
}
