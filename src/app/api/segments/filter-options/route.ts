...
export async function GET(request: Request) {
  try {
    ...
    const selectedBrandNames = brandIdsParam
      ? brandIdsParam
          .split(",")
          .map((b) => b.trim())
          .filter((b) => ALLOWED_BRANDS.has(b))
      : [];

    console.log(
      "[Filter Options API] Request received with brand_ids:",
      brandIdsParam,
    );
    console.log(
      "[Filter Options API] Selected brand names:",
      selectedBrandNames,
    );
    ...
    if (clients.status === "fulfilled") {
      const allowedClients = clients.value.filter((c) =>
        ALLOWED_BRANDS.has(c.name),
      );
      brands = allowedClients
        .map((c) => c.name)
        .sort((a, b) => a.localeCompare(b, "id"));
      allClientIds = allowedClients.map((c) => c.id);
      allowedClients.forEach((c) => clientMap.set(c.name, c.id));
      console.log("[Filter Options API] Available brands:", brands);
      console.log(
        "[Filter Options API] Client map:",
    ...
    if (selectedBrandNames.length > 0) {
      ...
      targetClientIds = selectedBrandNames
        .map((brandName) => {
          ...
          console.log(
            `[Filter Options API] Mapping brand "${brandName}" to client_id:`,
            clientId,
          );
          return clientId;
        })
      ...
      console.log(
        "[Filter Options API] Target client IDs for selected brands:",
        targetClientIds,
      );
    } else {
      ...
      console.log(
        "[Filter Options API] No brand filter, fetching all client IDs:",
        targetClientIds,
      );
    }
      ...
      try {
        ...
        const adsPlatformPromises = targetClientIds.map(async (clientId) => {
          try {
            ...
            console.error(
              `Failed to fetch ads platforms for client ${clientId}:`,
              err,
            );
            return [];
          }
        ...
        const csPromises = targetClientIds.map(async (clientId) => {
          try {
            ...
            console.error(
              `Failed to fetch customer services for client ${clientId}:`,
              err,
            );
            return [];
          }
        ...
        const [allProductsResults, allAdsPlatformsResults, allCSResults] =
          await Promise.all([
    ...
    const transactionTypes = [...TRANSACTION_TYPES].sort((a, b) =>
